...
**resmush** is an **R** package that lets users optimize and compress images
...
- Supported image formats are `png`, `jpg/jpeg`, `gif`, `bmp` and `tiff`.
...
<figcaption>Figure 2: Low-quality image due to high compression
(`qlty = 3`).</figcaption>
:::
...
- **xfun** [@xfun], which includes:
  ...
  - `xfun::optipng()`: Compresses local files using **OptiPNG**, which must be
    installed locally.
- [**tinieR**](https://jmablog.github.io/tinieR/) by jmablog: An **R** interface
  to [**TinyPNG**](https://tinypng.com/).
